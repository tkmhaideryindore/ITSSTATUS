...
async function searchSheet() {
  ...
  try {
    ...
    const response = await fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vQ4T5Sv0fLzVR-JVz5fbd9SEjxPexU_1ceanaiL2q9SlA-6OmsdzAxguAHFduN5PfQBmEGvXUf1FPrN/pub?gid=136588082&output=csv');
    ...
    if (matchingRow) {
      ...
      statusDisplay.textContent = `${fullName} - Status: <b>${status}<\b>`;
    } else {
